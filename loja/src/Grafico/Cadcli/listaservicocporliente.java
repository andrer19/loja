...
public class listaservicocporliente extends JDialog {
	...
	public listaservicocporliente(Long idclientep) throws Exception {
		...
		setTitle("LISTA ORDEM DE SERVIÇO ANTIGOS POR CLIENTE");
		...
		listaservicocliente.addColumn("SERVIÇO");
	...
	public void selecionapedido() {
		if (table.getSelectedRow() != -1 && table.getRowCount() > 0) {
			...
			try {
				...
				JOptionPane.showMessageDialog(null, "ERRO AO IMPRIMIR ORDEM DE SERVIÇO POR CLIENTE " + e1.getMessage());
				aces1.demologger.error("ERRO AO IMPRIMIR ORDEM DE SERVIÇO POR CLIENTE " + e1.getMessage());
			}
		}else {
